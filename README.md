...
**Note:** 
This pipeline was developed and executed on the University of Birmingham BlueBEAR  HPC cluster via SLURM batch submission. Notebooks provided 
for the core analyses (pseudobulk DE, Lasso regression, BACH2 co-expression, UMAP) display the exact code executed as static reference cells, alongside live cells that 
load and render the actual output files (CSVs, figures) generated by the batch runs. Remaining pipeline steps are provided as standalone, commented shell/Python/R scripts with matching SLURM submission scripts, run in the numbered order given below.
