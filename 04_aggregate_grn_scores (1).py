...
This script pools the GRNBoost2 'adjacencies.tsv' from every
...
def main(pyscenic_dir, outdir, top_n):
    ...
    for cell_type in sorted(plot_df.cell_type.unique()):
        ...
        fig, ax = plt.subplots(figsize=(max(10, 0.6 * len(order)), 7))
        ...
        ax.set_title(f"Top {top_n} most recurrent TF-target pairs — {cell_type}", fontsize=12)
